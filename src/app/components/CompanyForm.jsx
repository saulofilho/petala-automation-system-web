...
export default function CompanyForm({ onCreate }) {
  ...
  return (
    <section className={styles.companyForm} >
      <form className={styles.form} onSubmit={handleSubmit} noValidate>
        <Input label="Nome da empresa" name="name" value={form.name} onChange={handleChange} />
        {errors.name && <p className={styles.error}>{errors.name}</p>}

        <Input label="CNPJ da empresa" name="cnpj" value={form.cnpj} onChange={handleChange} maxLength={18} />
        {errors.cnpj && <p className={styles.error}>{errors.cnpj}</p>}

        <Input label="CEP da empresa" name="cep" value={form.cep} onChange={handleChange} maxLength={9} />
